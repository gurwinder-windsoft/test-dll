...
foreach ($collectionName in $collections) {
  ...
  & 'C:\mongodb-tools\mongodb-database-tools-windows-x86_64-100.10.0\bin\mongoexport.exe' --uri=$prodURI --db=SyncNotifyHubService --collection=$collectionName --out=$collectionExportPath --jsonArray
  ...
  & 'C:\mongodb-tools\mongodb-database-tools-windows-x86_64-100.10.0\bin\mongoimport.exe' --uri=$preprodURI --db=SyncNotifyHubService --collection=$collectionName --file=$collectionExportPath --jsonArray --upsert --verbose 
